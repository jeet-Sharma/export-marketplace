...
export interface RfqPreviewCardProps {
  rfq: PendingRfq;
...
export default function RfqPreviewCard({ rfq, onSendQuote, onViewDetails }: RfqPreviewCardProps) {
  return (
    <article className="p-4 border border-line rounded bg-panel">
      ...
      <div className="mt-4 flex items-center gap-2">
        <Button variant="accent" size="sm" onClick={() => onSendQuote?.(rfq)}>
          Send Quote
        </Button>
        <Button variant="ghost" size="sm" onClick={() => onViewDetails?.(rfq)}>
          Details
        </Button>
      </div>
    </article>
